...
class Help_slash(commands.Cog):
    ...
    def create_help_embeds(self, user: discord.User):
        ...
        embed2.add_field(name="warn", value="Avertir un membre /warn [@user] [raison] [nombre] (messages perms only)")
        embed2.add_field(name="resetwarn", value="Reset les warns d'un membre /resetwarn [@user] (messages perms only)")
        embed2.add_field(name="warnboard", value="Affiche le leaderboard des warns /warnboard")
        ...
        embed3.add_field(name="sayuser", value="Envoie un message à un utilisateur /sayuser [user] [message]")
